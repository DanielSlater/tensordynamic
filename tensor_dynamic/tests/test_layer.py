...
class TestLayer(TestCase):
    ...
    def test_create_layer(self):
        ...
        layer = self.LAYER_CLASS(InputLayer(input_p), output_nodes, session=self.session)
    ...
    def test_resize(self):
        ...
        input_p = tf.placeholder("float", (None, 10))
        layer = self.LAYER_CLASS(InputLayer(input_p), output_nodes, session=self.session)
        layer.resize(output_nodes + 1, self.session)
    ...
    def test_get_layers_list(self):
        input_p = tf.placeholder("float", (None, 10))
        layer = self.LAYER_CLASS(InputLayer(input_p), 1, session=self.session)
        layer2 = self.LAYER_CLASS(layer, 2, session=self.session)
        layer3 = self.LAYER_CLASS(layer2, 3, session=self.session)
